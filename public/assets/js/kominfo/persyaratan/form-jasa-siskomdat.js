...
// Class definition
var form_jasa_siskomdat = function () {
	// Elements		
	var number = 1;
	...
	var initForm = () => {
		...
		$("#simpan-jasa-siskomdat").click(function() {	
			// event.preventDefault();
			$.ajax({
				url: 'proses-jasa-siskomdat',
				type: 'post',
				dataType: 'json',
				// data: $('form#form_data_alat_teknis').serialize(),
                headers: {"X-CSRF-TOKEN": $("meta[name='csrf-token']").attr("content")},
				data: new FormData(document.getElementById("form_jasa_siskomdat")),
				processData: false,
				contentType: false,
        ...
        $("#tambah-jasa-siskomdat").click(function() {			
			$("#body-jasa-siskomdat").append(foRow);
            initiateKabKota();
		});
		
		$("#body-jasa-siskomdat").on('click','.remove',function(){
		...
		var foRow = `
                <tr>
                    ...
                    <button id="jasa-siskomdat-delete-`+ (number) + `" class="btn btn-icon btn-danger remove w-30px h-30px">
