...
var form_jasa_isp = function () {
	...
	var initForm = () => {
		...
		$("#simpan-jasa-isp").click(function() {	
			...
			$.ajax({
				...
				data: new FormData(document.getElementById("form_jasa_isp")),
				processData: false,
				contentType: false,
        ...
        $("#tambah-jasa-isp").click(function() {			
			$("#body-jasa-isp").append(foRow);
            initiateKabKota();
		});
		
		$("#body-jasa-isp").on('click','.remove',function(){
		...
		var foRow = `
                <tr>
                    ...
                    <td><select id="cakupan-wilayah[]" name="cakupan-wilayah[]" class="form-control form-select pusat_pelayanan_kota" data-control="select2"></select></td>
                    <td><input type="text" name="kapasitas_bw[]" class="form-control" placeholder=""/></td>
                    <td class="items-align-center text-end">
                    <button id="jasa-isp-delete-`+ (number) + `" class="btn btn-icon btn-danger remove w-30px h-30px">
