...
<div id="daftar_perangkat_lists">
									...
									<div class="col-md-12">
										<div class="form-group {{ $d['sertifikat_perangkat'] ?? 'hidden' }}">
											...
											<input class="form-control sertifikat_perangkat"
												name="daftar_perangkat[{{ $key }}][sertifikat_perangkat]" value="{{ $d['sertifikat_perangkat'] }}"
	...
	@else
		...
		// dd($datajson);
		?>
		{{-- <div class="device-form-row mb-3" data-index="0"> --}}
			...
			<div>
				<div>
					<small for="" class="text-danger mr-2">*Catatan : Untuk beberapa perangkat, memiliki kententuan yang
						harus didaftarkan pada Layanan Sertifikasi <a href='https://sertifikasi.postel.go.id'>
...
@if ($datajson == 'kosong' || $is_editable)
	<div class="mt-2 text-right">
		<button type="button" class="btn-secondary btn-sm" id="add_perangkat">Tambah Data
...
<script nonce="unique-nonce-value">
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		container.addEventListener('click', function(event) {
			if (event.target && event.target.classList.contains('add_perangkat')) {
				addDeviceRow_(event.target);
			}
		});
		$('#add_perangkat').on('click', function() {
		...
		function addDeviceRow_() {
			...
			const col1 = createColumn([
				createFormGroup('No Sertifikat Perangkat', 'daftar_perangkat', deviceIndex,
					'sertifikat_perangkat', 'text', null, false, 'hidden'),
				createFormGroup('Lampiran Sertifikasi Perangkat', 'daftar_perangkat', deviceIndex,
					'sertifikasi_alat', 'file', 'application/pdf', false, 'hidden'),
			], 'col-md-12');

			const col2 = createColumn([
				createFormGroup('Lokasi Perangkat', 'daftar_perangkat', deviceIndex, 'lokasi_perangkat',
					'textarea', null, true),
				createFormGroup('Jenis Perangkat', 'daftar_perangkat', deviceIndex, 'jenis_perangkat'),
				createFormGroup('Merk Perangkat', 'daftar_perangkat', deviceIndex, 'merk_perangkat'),
				createFormGroup('Nomor Serial Number Perangkat', 'daftar_perangkat', deviceIndex,
					'sn_perangkat'),
				createFormGroup('Foto Perangkat', 'daftar_perangkat', deviceIndex, 'foto_perangkat',
					'file', 'application/pdf'),
			], 'col-md-6');

			const col3 = createColumn([
				createFormGroup('Tipe Perangkat', 'daftar_perangkat', deviceIndex, 'tipe_perangkat'),
				createFormGroup('Negara Asal Pembuat Perangkat', 'daftar_perangkat', deviceIndex,
					'buatan_perangkat'),
				createFormGroup('Foto Serial Number Perangkat', 'daftar_perangkat', deviceIndex,
	...
	$(document).on('click', '.switchPerangkat', function() {
		...
		if ($(this).is(':checked')) {
			...
			perangkat.find('input.form-control[name="daftar_perangkat[' + index +
