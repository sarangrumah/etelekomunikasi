	...
	<table class="table table-custom table-sm">
		<thead>
			<tr>
				...
				<th class="text-center">Kapasitas Transponder Satelit yang disewakan (MHz/Mbps)</th>
				<th class="text-center">Kapasitas Sistem (SSM)</th>
			</tr>
		</thead>
					...
					@if (isset($d['isdeleted']))
						@if ($d['isdeleted'] == '1')
							...
							<tr class="rolloutplan-item">
								...
								<td class="col-4">
									<input type="text" {{ $is_editable ? '' : 'readonly' }} value="{{ $d['kapasitas_transponder_satelit'] }}"
										class="form-control" name="rolloutplan[{{ $key }}][kapasitas_transponder_satelit]" required />
								</td>
								<td class="col-4">
									<input type="number" min="1" {{ $is_editable ? '' : 'readonly' }} value="{{ $d['kapasitas_sistem'] }}"
										class="form-control" name="rolloutplan[{{ $key }}][kapasitas_sistem]" required />
		...
		const addRolloutPlanItem = function() {
			...
			$('#add-RolloutPlan').on('click', function() {
				...
				const inputRow =
					...
					`][kapasitas_transponder_satelit]" required />
						</td>
						<td class="col-4">
