	...
	<table class="table table-custom table-sm">
		<thead>
			<tr>
				...
				<th class="text-center">Kapasitas Transponder yang disewa/disediakan (MHz/Mbps)</th>
			</tr>
		</thead>
					...
					@if (isset($d['isdeleted']))
						@if ($d['isdeleted'] == '1')
							...
							<tr class="rolloutplan-item">
								...
								<td class="col-6">
									<input type="text" value="{{ $d['kapasitas_transponder'] }}" {{ $is_editable ? '' : 'readonly' }}
										class="form-control" name="rolloutplan[{{ $key }}][kapasitas_transponder]" required />
			...
			@else
				@for ($i = 0; $i < 5; $i++)
					<tr class="rolloutplan-item">
						...
						<td class="col-6">
							<input type="text" class="form-control" name="rolloutplan[{{ $i }}][kapasitas_transponder]"
								required />
