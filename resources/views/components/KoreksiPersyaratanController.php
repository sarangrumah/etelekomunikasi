    ...
    public function submitpersyaratan(Request $req)
    {
        ...
        if (isset($req->cakupanwilayahtelsus_skrd)) {
            ...
            $skrd = DB::table('tb_trx_persyaratan')
                ->select('id')->where('id_map_listpersyaratan', '=', $req->id_maplist_cakupanwilayahtelsus_skrd)
        ...
        //         'is_active' => '1'
        //     ];
        //     $rencanausaha = DB::table('tb_trx_persyaratan')
        //                 ->select('id')->where('id_map_listpersyaratan', '=', $req->id_maplist_rencanausaha)
        //                 ->where('id_trx_izin', '=', $req->id_izin)
        //                 ->update($updatePerencanaan);
        // }
        if (isset($req->rencanausaha)) {
            $updateRencanaUsaha = [
                'id_trx_izin' => $req->id_izin,
        ...
        // ->where('id_map_listpersyaratan', '=', $req->id_maplist_rencanausaha)
        // ->where('id_trx_izin', '=', $req->id_izin);
        // $mtk = $mtk->update($updateRencanaUsaha);
        // }
        ...
        //     //     }
        //     // }
        //     $updateDaftarPerangkat['filled_document'] = json_encode($newkofig);
        //     $daftar_perangkat = $konftek->update($updateDaftarPerangkat);
        // }
        // dd($req->daftar_perangkat, $req->daftar_perangkat_telsus);
        if (isset($req->daftar_perangkat)) {
        ...
        //     "nib_user" => $izin_persyaratan->nib,
        //     "tgl_submit" => date('d F Y'),
        //     "nama_perseroan" => $nib->nama_perseroan,
        //     "jenis_izin" => $izin_persyaratan->jenis_izin,
        //     "kbli" => $izin_persyaratan->kbli,
        //     "kbli_name" => $izin_persyaratan->kbli_name
        // ];
        // $user = Auth::User()->email;
        // $koordinator = MstUserBo::where('username', 'koordinatorjasa')->first();
        // $koordinator = $koordinator->email;
