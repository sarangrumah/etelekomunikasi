					...
					@if (isset($d['isdeleted_rencanausaha']))
						@if ($d['isdeleted_rencanausaha'] == '1')
							<tr class="rencanausaha-item">
								...
								<td style="width: 20%;">
									<select {{ $is_editable ? '' : 'readonly' }} name="rencanausaha[{{ $key }}][node]" id=""
										class="form-control node-rencanausaha select2">
										<option value="">Pilih kota</option>
										@foreach ($cities as $city)
											<option value="{{ $city->id }}"{{ $d['node'] == $city->id ? 'selected' : '' }}>
												{{ $city->name }}</option>
										@endforeach
									</select>
								</td>
								<td>
									<input {{ $is_editable ? '' : 'readonly' }} value="{{ $d['band_domestik'] }}" type="number" min="0"
										class="form-control band_domestik-rencanausaha" name="rencanausaha[{{ $key }}][band_domestik]" />
								</td>
								<td>
									<input {{ $is_editable ? '' : 'readonly' }} value="{{ $d['band_inter'] }}" type="number" min="0"
										class="form-control band_inter-rencanausaha" name="rencanausaha[{{ $key }}][band_inter]" />
								</td>
								<td style="width: 10%;">
									<div>
