...
<div style="padding: 60px 0 0 0;">
    ...
    <div style="width: 100vw;margin-left: calc(50% - 50vw);background: #E5E5E5;padding: 150px 140px;margin-top: -100px;min-height: 100vh;">
        <form method="POST" action="{{ route('form-submit', Request::segment(3)) }}" class="form-horizontal">
        @csrf
        @foreach ($question as $q)
                ...
                <div class="col-6 row inline">
                    <div class="col-3">
                        <label class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="1" {{$q['survey_result']=='1'?"checked":''}}>
                            <span class="custom-control-label"></span>
                        </label>
                    </div>
                    <div class="col-3">
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="2" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label"></span>
                    </label>
                    </div>
                    <div class="col-3">
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="3" {{$q['survey_result']=='3'?"checked":''}}>
                        <span class="custom-control-label"></span>
                    </label>
                    </div>
                    <div class="col-3">
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="4" {{$q['survey_result']=='4'?"checked":''}}>
                        <span class="custom-control-label"></span>
                    </label>
                    </div>
                </div>
            ...
            @elseif($q['cat']==4)
                <h3>{{ $loop->iteration }}. {{$q['question_name']}}</h3>
                    ...
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="3" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">WAKTU PENYELESAIAN</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="4" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">BIAYA/TARIF</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="5" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">PRODUK SPESIFIKASI JENIS PELAYANAN</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="6" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">KOMPETENSI PELAKSANA</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="7" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">PERILAKU PELAKSANA</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="8" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">PENANGANAN PENGADUAN, SARAN DAN MASUKAN</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="9" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">SARANA DAN PRASARANA</span>
                    </label>
                    @endif
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="q[{{$q['id_map']}}]" value="10" {{$q['survey_result']=='2'?"checked":''}}>
                        <span class="custom-control-label">UMUM</span>
                    </label>
                    </div>
                    <div class="pertanyaan">
                        <textarea name="pertanyaan[{{$q['id_map']}}]" class="form-control">{{$q['survey_answer']}}</textarea>
                    </div>
            @else
                <h3>{{ $loop->iteration }}. {{$q['question_name']}}</h3>
                @if($q['id_tb_mst_question_type']==1 || $q['id_tb_mst_question_type']==2)
                    ...
                    <label class="custom-control">
                        <textarea class="form-control" name="q[{{$q['id_map']}}]"></textarea>
                    </label>
                @endif
            @endif
            <br><br>
