...
class DirekturController extends Controller
{
    ...
    public function index(){
        ...
        ->leftjoin('vw_list_izin as v','t.id_izin','=','v.id_izin')->with('KodeIzin')
        ->with('KodeAkses')->with('KodeAkses.JenisKodeAkses')->take($limit_db);
        ...
        // dd($ulo);
        return
        view('layouts.backend.direktur.dashboard',['date_reformat'=>$date_reformat,'ulo'=>$ulo,'countulo'=>$countulo,'penomoran'=>$penomoran,'countpenomoran'
        => $countpenomoran,'izinprinsip'=>$izinprinsip,'countip' => $countip,
    ...
    public function penetapanIPPost($id,Request $request){
        ...
        ->whereIn('status_checklist',[804])->first();
        $penanggungjawab = array();
    ...
    public function penetapanUloPost($id,Request $request){
        ...
        // try {
            ...
            $putfile = $this->putFileSK($id);
            $putfile2 = $this->putFileSKPenetapan($id);
            // $datenow = Carbon::now();
            // $common = new CommonHelper;
            ...
            // $nama2 = $evaluator->nama;
            // // dd($ulo);
            // $kirim_email = $email->kirim_email($penanggungjawab,$email_jenis,$ulo,$departemen,$catatan_hasil_evaluasi,$nama2,$nibs,$koreksi_all,$attachfile, '', $attachfile2);
    ...
    public function kirimSK($id,Request $request){
        ...
        DB::beginTransaction();
        try {
            $kirim_email = $email->kirim_email($penanggungjawab,$email_jenis,$ulo,$departemen,'',$nama2,$nibs,$koreksi_all,$attachfile, '', $attachfile2);
            DB::commit();
        } catch (\Exception $e) {
            //     // dd($e);
                DB::rollback();
    ...
    public function putFileSKPenetapan($id_izin)
    {
        ...
        if ($id_master_izin == 1) {
            ...
            $latest_nokomitmen = $nokomitmen->izinkomitmenno;
            $pdf = PDF::loadView('layouts.backend.sk.cetak-penetapan', ['ulo'=>$ulo,'latest_nokomitmen'=>$latest_nokomitmen,'idizin'=>$idizin,'nama_perseroan' => $nama_perseroan, 'data' => $data, 'cities' => $cities, 'map_izin_perubahan' =>$map_izin_perubahan, 'mst_kode_izin' => $mst_kode_izin]);
        }
