												...
												<div class="dropdown">
													...
													<div class="dropdown-menu dropdown-menu-right">
														@if ($loges->oss_kode != '00')
															<a href="{{ route('admin.view', [$loges->id_izin, $loges->id]) }}" class="dropdown-item"><i
																	class="icon-pencil"></i> Lihat Detil Perizinan</a>
														@endif
														@if (isset($loges->skulo))
															<a target="_blank" href="{{ asset($loges->skulo) }}" class="dropdown-item"><i class="icon-pencil"></i> SK
																ULO</a>
														@endif
														@if (isset($loges->skkomit))
															@if (substr($loges->id_izin, 0, 3) == 'TKI')
																<a target="_blank" href="{{ asset($loges->skkomit) }}" class="dropdown-item"><i class="icon-pencil"></i>
																	SK Izin Penyelenggaraan</a>
															@else
																<a target="_blank" href="{{ asset($loges->skkomit) }}" class="dropdown-item"><i class="icon-pencil"></i>
																	SK Komitmen</a>
															@endif
