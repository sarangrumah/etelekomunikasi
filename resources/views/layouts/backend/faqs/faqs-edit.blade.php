...
@section('content')
	<form method="post" id="formFaq" action="{{ route('admin.editfaqpost', [$faqs['id']]) }}" enctype="multipart/form-data">
		@csrf
		<div class="form-group">
			...
			<div class="card">
				<div class="card-header bg-indigo text-white header-elements-inline">
					<div class="row">
						<div class="col-lg">
							<h6 class="card-title font-weight-semibold py-3">Edit FAQs</h6>
						</div>
					</div>
				</div>
										...
										<div class="col-6">
											...
											<select id="faq_types" id="faq_types" name="faq_types" class="form-control">
												...
												@foreach ($faq_type as $faq_types)
													...
													<option value="{{ $faq_types->short_desc }}" @if ($faqs['type'] == $faq_types->short_desc) selected="selected" @endif>
														{{ $faq_types->desc }}</option>
												@endforeach
											</select>
										...
										<div class="col-6">
											...
											<select id="faq_category" id="faq_category" name="faq_category" class="form-control">
												...
												@foreach ($faq_category as $faq_categories)
													...
													<option value="{{ $faq_categories->short_desc }}"
														@if ($faqs['category'] == $faq_categories->short_desc) selected="selected" @endif>
														{{ $faq_categories->desc }}</option>
												@endforeach
											</select>
									...
									<div class="row">
										<div class="col-lg-6">
											...
											{{-- <input type="file" class="form-control h-auto" name="faq_document" id="faq_document"
												accept="application/pdf" placeholder="{{ $faqs['download_link'] }}"> --}}

											<?php
											if (isset($faqs['download_link']) && $faqs['download_link'] != '') {
											    $_file_berkas = explode('/', $faqs['download_link']);
											...
											<div class="col-lg-12">
												@if ($file_berkas == '')
													<input type="hidden" name="download_link_prv" id="download_link_prv"
														value="{{ asset($faqs['download_link']) }}">
												@endif
												<div class="input-group">
													<input type="file" class="form-control h-auto" name="download_link" id="download_link">
													<span class="input-group-append">
														@if ($file_berkas == '')
															{{-- <a target="_blank" href="#" class="btn btn-teal" type="button">Lihat
																Dokumen</a> --}}
														@else
															<a target="_blank" href="{{ asset($faqs['download_link']) }}" class="btn btn-teal" type="button">Lihat
																Dokumen</a>
														@endif
													</span>
												</div>
										...
										<div class="col-6">
											...
											<select id="faq_status" name="faq_status" class="form-control">
												...
												<option value="1" @if ($faqs['is_active'] == '1') selected="selected" @endif>Aktif</option>
												<option value="0" @if ($faqs['is_active'] == '0') selected="selected" @endif>Tidak Aktif</option>
											</select>
										</div>
									</div>
								</div>
								...
								<div class="col-lg">
									<div class="row">
										<div class="col-12">
											...
											<textarea class="form-control" id="faq_question" name="faq_question" rows="3">{{ $faqs['question'] }}</textarea>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="col-lg">
									<div class="row">
										<div class="col-12">
											...
											<textarea class="form-control" id="faq_answer" name="faq_answer" rows="3">{{ $faqs['answer'] }}</textarea>
										</div>
									</div>
								</div>
							</div>
