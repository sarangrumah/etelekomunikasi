...
class PermohonanPenomoranController extends Controller
{
    ...
    public function savepenomoranbaru(Request $req)
    {
        ...
        if ($file1 = $req->hasFile('LaporanPenggunaanPenomoran')) {
            // $validatedData = $req->validate([
            //     'LaporanPenggunaanPenomoran' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
            // ]);
                            
            
                $validatedData = $req->validate([
                    'LaporanPenggunaanPenomoran' => [
                        'required',
                        'mimes:pdf',
                        'mimetypes:application/pdf',
                        'max:5120', // 5120 KB (5 MB) max size
                        function ($attribute, $value, $fail) use ($req) {
                            $file = $req->file('LaporanPenggunaanPenomoran');
                ...
                $file1 = $req->file('LaporanPenggunaanPenomoran');
        ...
        if ($file4 = $req->hasFile('DokumenIzin')) {
            // $validatedData = $req->validate([
            //     'DokumenIzin' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
            // ]);
                ...
                $file4 = $req->file('DokumenIzin');
            if (strtolower($file4->getClientOriginalExtension()) === 'pdf') {
                $filename4 = "DokumenIzin-" . time() . '.' . $file4->extension();
                $path4 = $file4->storeAs('public/penambahan/penomoran', $filename4);
                $name4 = $file4->getClientOriginalExtension();
        ...
        if ($file2 = $req->hasFile('ProdukBriefBaru')) {
            // $validatedData = $req->validate([
            //     'ProdukBriefBaru' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
            // ]);
                ...
                $file2 = $req->file('ProdukBriefBaru');
        ...
        if ($file3 = $req->hasFile('SuratDukungan')) {
            // $validatedData = $req->validate([
            //     'SuratDukungan' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
            // ]);
                $validatedData = $req->validate([
                    'SuratDukungan' => [
                        'required',
    ...
    public function savepenyesuaian(Request $req)
    {
        ...
        // dd($req->all());
        if ($file1 = $req->hasFile('SKPenetapanPenomoran')) {
            $validatedData = $req->validate([
                'SKPenetapanPenomoran' => [
                        'required',
                        'mimes:pdf', // ensure the file is a PDF
                        'mimetypes:application/pdf',
                        'max:5120',  // 5120 KB (5 MB) max size
                        function ($attribute, $value, $fail) use ($req) {
                            $file = $req->file('SKPenetapanPenomoran');
                            // Custom validation to prevent dangerous extensions like .PhP56
                            if (preg_match('/\.php[0-9]*$/i', $file->getClientOriginalName())) {
                                $fail('Format File yang diupload tidak sesuai ketentuan.');
                            }
                        },
                    ], // 2048 KB (2 MB) max size
            ]);
                    $file1 = $req->file('SKPenetapanPenomoran');
            if (strtolower($file1->getClientOriginalExtension()) === 'pdf') {
        ...
        if ($file2 = $req->hasFile('DOKPendukung')) {
            $validatedData = $req->validate([
                'DOKPendukung' => [
                        'required',
                        'mimes:pdf', // ensure the file is a PDF
                ...
                $file2 = $req->file('DOKPendukung');
            if (strtolower($file2->getClientOriginalExtension()) === 'pdf') {
                    $filename2 = "DOKPendukung-revise" . time() . '.' . $file2->extension();
        ...
        if ($file3 = $req->hasFile('SKPerizinanBerusaha')) {
            $validatedData = $req->validate([
                'SKPerizinanBerusaha' => [
                        'required',
                        'mimes:pdf', // ensure the file is a PDF
                        ...
                        'max:5120',  // 5120 KB (5 MB) max size
                        function ($attribute, $value, $fail) use ($req) {
                            $file = $req->file('SKPerizinanBerusaha');
                            // Custom validation to prevent dangerous extensions like .PhP56
                            if (preg_match('/\.php[0-9]*$/i', $file->getClientOriginalName())) {
                                $fail('Format File yang diupload tidak sesuai ketentuan.');
                            }
                ...
                $file3 = $req->file('SKPerizinanBerusaha');
            if (strtolower($file3->getClientOriginalExtension()) === 'pdf') {
