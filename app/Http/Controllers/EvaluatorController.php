...
class EvaluatorController extends Controller
{
    ...
    public function jasa(Request $request)
    {
        ...
        if ($request->filled('search')) {
            $izin->where('vw_list_izin.nama_perseroan', 'like', '%' . $request->search . '%');
        }
        if ($request->filled('status')) {
            $izin->where('vw_list_izin.status_checklist', $request->status);
        }
        if ($request->filled('date_from') && $request->filled('date_to')) {
            $izin->whereBetween('vw_list_izin.created_at', [$request->date_from, $request->date_to]);
        }

        $izin = $izin->where('vw_list_izin.id_master_izin', '=', $id_departemen_user)
            ->distinct('vw_list_izin.id_izin')
        ...
        return view('layouts.backend.evaluator.dashboard', [
    ...
    public function telsus(Request $request)
    {
        ...
        $izin = $izin->distinct('vw_list_izin.id_izin')
            ->orderBy('updated_at', 'desc')
            ->paginate($limit_db);
    ...
    public function ulo(Request $request)
    {
        ...
        $jenis_izin = 'Izin Penyelenggaraan Jaringan Telekomunikasi';
        return view('layouts.backend.evaluator.dashboard-ulo', [
            'date_reformat' => $date_reformat,
            'ulo' => $ulo,
            'paginate' => $paginate,
            'jenis_izin' => $jenis_izin,
