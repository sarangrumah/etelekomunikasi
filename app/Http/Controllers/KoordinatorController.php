...
class KoordinatorController extends Controller
{
    ...
    public function index(){
        ...
        if ($id_departemen_user == 1) {
            $result = Redirect::route('admin.koordinator.jasa');
        }else if($id_departemen_user == 2){
            $result = Redirect::route('admin.koordinator.jaringan');
        }else if($id_departemen_user == 4){
            $result = Redirect::route('admin.koordinator.ulo');
        }else if($id_departemen_user == 5){
            $result = Redirect::route('admin.koordinator.penomoran');
        }else{
            $result = Redirect::route('admin.koordinator.telsus');
        }

        // --- PROFILING END ---
        $endMemory = memory_get_usage();
        $queryCount = count(\DB::getQueryLog());
        \Log::info('PROFILE: KoordinatorController@index', [
            'queries' => $queryCount,
    ...
    public function jasa(Request $request){
        ...
        return view('layouts.backend.koordinator.dashboard', [
    ...
    public function ulo(Request $request){
        ...
        return view('layouts.backend.koordinator.dashboard-ulo', [
            'date_reformat' => $date_reformat,
            'paginate' => $paginate,
            'ulo' => $ulo,
            'countdisposisi' => $countdisposisi,
    ...
    public function penomoran(Request $request){
        ...
        $penomoran = Penomoran::from('tb_trx_kode_akses as t')
            ->select('t.id as id_kode_akses','t.*','v.*', 'z.name_status_fo as penomoran_fo', 'z.name_status_bo as penomoran_bo', 'y.kode_akses')
        ...
        return view('layouts.backend.koordinator.dashboard-penomoran', [
            'date_reformat' => $date_reformat,
            'penomoran' => $penomoran,
            'paginate' => $paginate,
            'jenis_izin' => $jenis_izin,
