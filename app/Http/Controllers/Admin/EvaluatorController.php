...
class EvaluatorController extends Controller
{
    ...
    public function ulo()
    {
        ...
        //     return abort(404);
        // }
        if (!in_array(Session::get('id_mst_jobposition'), [12, 24])) {
            return abort(404);
        }
        $id_user_session = Session::get('id_user');
        // $limit_db = Config::get('app.admin.limit');
    ...
    public function saveHasilEvaluasiUlo($id, $urut, Request $request)
    {
        ...
        if (isset($uloSave['nomor_sklo'])) {
            ...
            if ($uloSave['jenis_izin'] == 'JASA') {
                ...
                $init = 'TELSUS';
                $nomorsklo = DB::table('latest_sklono_telsus')->select('*')->first();
                // $nomorsklo = DB::table('latest_sklono')->select('*')->first();
                // $nomorsklo_fixed = $init . '-' . $nomorsklo_fixed;
                $nomorsklo_fixed = $nomorsklo_fixed;
            }
        ...
        if (isset($uloSave['status_hasil_evaluasi'])) {
            if ($uloSave['status_hasil_evaluasi'] == 1) {
                ...
                $uloSave->save();
                DB::commit();
                return redirect()->route('admin.evaluator.evaluasi-ulo', [$id, $urut])
                         ->with('message', 'Akses rubah hasil evaluasi Uji Laik Operasi telah Berhasil.');
            } else {
                ...
                // FILE
                if ($request->hasFile('uploadSuratPerintahTugas')) {
                    $date_reformat = new DateHelper();
                    // $validatedData = $request->validate([
                    //     'uploadSuratPerintahTugas' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                    // ]);
                    
                        // $validatedData = $request->validate([
                        //     'uploadSuratPerintahTugas' => [
                        //         'required',
                        //         'mimes:pdf',
                        //         'mimetypes:application/pdf',
                        //         'max:104857600', // 5120 KB (5 MB) max size
                        //         function ($attribute, $value, $fail) use ($request) {
                        //             $file = $request->file('uploadSuratPerintahTugas');
                        ...
                        //     ],
                        // ]);
                    $file1 = $request->file('uploadSuratPerintahTugas');
                    if (strtolower($file1->getClientOriginalExtension()) === 'pdf') {
                        ...
                        return redirect()->route('admin.evaluator')->with('message', 'Format File yang diupload tidak sesuai ketentuan.');
                    }
                ...
                if ($request->hasFile('UploadDokumenHasilEvaluasiPelaksanaanUlo')) {
                    $date_reformat = new DateHelper();
                    // $validatedData = $request->validate([
                    //     'UploadDokumenHasilEvaluasiPelaksanaanUlo' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                    // ]);
                    
                        // $validatedData = $request->validate([
                        //     'UploadDokumenHasilEvaluasiPelaksanaanUlo' => [
                        //         'required',
                        //         'mimes:pdf',
                        //         'mimetypes:application/pdf',
                        //         'max:104857600', // 5120 KB (5 MB) max size
                        //     function ($attribute, $value, $fail) use ($request) {
                        //         $file = $request->file('UploadDokumenHasilEvaluasiPelaksanaanUlo');
                        //             // Custom validation to prevent dangerous extensions like .PhP56
                        //             if (preg_match('/\.php[0-9]*$/i', $file->getClientOriginalExtension())) {
                        //                 $fail('Invalid file extension.');
                        //             }
                        ...
                        //     ],
                        // ]);
                    $file2 = $request->file('UploadDokumenHasilEvaluasiPelaksanaanUlo');
                        $validatedData = $request->validate([
                            'UploadDokumenHasilEvaluasiPelaksanaanUlo' => [
                                'required',
                                'mimes:pdf',
                ...
                if (isset($data['MediaTransmisiYangDigunakan'])) {
                    ...
                    $uloSave->media_transmisi_yang_digunakan = $ulo['media_transmisi_yang_digunakan'];
                }
                if (isset($data['alamatPusatLayananPelangan'])) {
                    ...
                    $uloSave->alamat_pusat_layanan_pelangan = $ulo['alamat_pusat_layanan_pelangan'];
                }
                if (isset($data['AlamatPelaksanaanUlo'])) {
                    $uloSave->alamat_pelaksanaan_ulo = isset($data['AlamatPelaksanaanUlo']) ? $data['AlamatPelaksanaanUlo'] : '';
                }else{
                    $uloSave->alamat_pelaksanaan_ulo = $ulo['alamat_pelaksanaan_ulo'];
                }
                // $uloSave->alamat_pelaksanaan_ulo = $data['AlamatPelaksanaanUlo'];
                $uloSave->tanggal_evaluasi_pelaksanaan_ulo = $data['TanggalEvaluasiPelaksanaanUlo'];
                $uloSave->no_surat_perintah_tugas = $data['NoSuratPerintahTugas'];
                $uloSave->tanggal_surat_perintah_tugas = $data['TanggalSuratPerintahTugas'];
            ...
            if(!isset($request->status_laik)){
                return redirect()->route('admin.evaluator.evaluasi-ulo', [$id, $urut])
                         ->with('message', 'Anda belum memilih Status Laik.');
            }
            if ((isset($data['is_koreksi_surat_permohonan']) && $data['is_koreksi_surat_permohonan'] == 'on')
                || isset($data['is_koreksi_surat_tugas']) && $data['is_koreksi_surat_tugas'] == 'on'
                || isset($data['is_koreksi_surat_tugas']) && $data['is_koreksi_surat_tugas'] == 'on'
        ...
        // return Redirect::route('admin.evaluator')->with('message', 'Evaluasi berhasil di proses');
        return redirect()->route('admin.evaluator.evaluasi-ulo', [$id_izin, $urut])
                     ->with('message', 'Berhasil Menyimpan Hasil Evaluasi.');
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function evaluasiUloPost($id, $urut, Request $request)
    {
        ...
        // END FILE
        // dd($uloSave);
        if ($request->hasFile('dok_surat_permohonan_ulo_asli') != null) {
            ...
            if ($dok_surat_permohonan_ulo_asli = $request->file('dok_surat_permohonan_ulo_asli')) {
                // $validatedData = $dok_surat_permohonan_ulo_asli->validate([
                //     'dok_surat_permohonan_ulo_asli' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                // ]);
                //     $validatedData = $request->validate([
                //         'dok_surat_permohonan_ulo_asli' => [
                //             'required',
                //             'mimes:pdf',
                //             'mimetypes:application/pdf',
                //             'max:104857600', // 5120 KB (5 MB) max size
                //             function ($attribute, $value, $fail) use ($request) {
                // $file = $request->file('dok_surat_permohonan_ulo_asli');
                //                 // Custom validation to prevent dangerous extensions like .PhP56
                //                 if (preg_match('/\.php[0-9]*$/i', $file->getClientOriginalExtension())) {
                //                     $fail('Invalid file extension.');
                //                 }
                ...
                //         ],
                //     ]);
                $dok_surat_permohonan_ulo_asli = $request->file('dok_surat_permohonan_ulo_asli');
                if (strtolower($dok_surat_permohonan_ulo_asli->getClientOriginalExtension()) === 'pdf') {
                    $request->validate(["dok_surat_permohonan_ulo_asli" => 'mimes:pdf|mimetypes:application/pdf|max:5120',
                                ]);
                    ...
                    // dd($path_konfigurasi_sistem);
                    $name_dok_surat_permohonan_ulo_asli = $dok_surat_permohonan_ulo_asli->getClientOriginalExtension();
        ...
        //                 $name_foto_perangkat = $foto_perangkat->getClientOriginalExtension();
                ...
                foreach ($request->daftar_perangkat as $key => $file) {
                    ...
                    if (isset($request->prv_foto_perangkat[$key])) {
                        ...
                        if (isset($request->daftar_perangkat[$key]['foto_perangkat'])) {
                            // $validatedData = $request->validate([
                            //     'foto_perangkat' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                            // ]);
        ...
        // dd('ewat');
        if ($request->hasFile('konfigurasi_sistem') != null) {
            ...
            if ($request->hasFile('konfigurasi_sistem')) {
                ...
                if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                    $request->validate([
                        "konfigurasi_sistem" => 'mimes:pdf|mimetypes:application/pdf|max:5120',
                    ]);
        ...
        if ($request->file('bukti_perangkat') != null) {
            ...
            if ($request->hasFile('bukti_perangkat')) {
                ...
                // if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                //     $request->validate([
                //         "bukti_perangkat" => 'mimes:pdf|mimetypes:application/pdf|max:5120',
                //     ]);
        ...
        if ($request->hasFile('dok_layanan_prapurna_jual') != null) {
            $id_dok_layanan_prapurna_jual = $data['id_dok_layanan_prapurna_jual'];
            $path_dok_layanan_prapurna_jual = '';
            if ($request->hasFile('dok_layanan_prapurna_jual')) {
                // $validatedData = $request->validate([
                //     'dok_layanan_prapurna_jual' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                // ]);
                $file = $request->file('dok_layanan_prapurna_jual');
                // if (strtolower(($file->getClientOriginalExtension()) == 'pdf')) {
                //     $validatedData = $file->validate([
                //         'dok_layanan_prapurna_jual' => [
                //             'required',
                //             'mimes:pdf',
                //             'mimetypes:application/pdf',
                //             'max:104857600', // 5120 KB (5 MB) max size
                ...
                if (strtolower(($file->getClientOriginalExtension()) == 'pdf')) {
                    $request->validate([
                                    "dok_layanan_prapurna_jual" => 'mimes:pdf|mimetypes:application/pdf|max:5120',
                                ]);
                    $dok_layanan_prapurna_jual = $request->file('dok_layanan_prapurna_jual');
                    ...
                    $name_dok_layanan_prapurna_jual = $dok_layanan_prapurna_jual->getClientOriginalExtension();
        ...
        if ($request->hasFile('dok_peta_jaringan') != null) {
            $id_dok_peta_jaringan = $data['id_dok_peta_jaringan'];
            $path_dok_peta_jaringan = '';
            if ($request->hasFile('dok_peta_jaringan')) {
                // $validatedData = $request->validate([
                //     'dok_peta_jaringan' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                // ]);
                $file = $request->file('dok_peta_jaringan');
                // if (strtolower(($file->getClientOriginalExtension()) == 'pdf')) {
                //     $validatedData = $file->validate([
                //         'dok_peta_jaringan' => [
                //             'required',
                ...
                if (strtolower(($file->getClientOriginalExtension()) == 'pdf')) {
                    $request->validate([
                                    "dok_peta_jaringan" => 'mimes:pdf|mimetypes:application/pdf|max:5120',
                                ]);
                    $dok_peta_jaringan = $request->file('dok_peta_jaringan');
                    ...
                    $name_dok_peta_jaringan = $dok_peta_jaringan->getClientOriginalExtension();
        ...
        // return Redirect::route('admin.evaluator.ulo');
        return redirect()->route('admin.evaluator.evaluasi-ulo', [$id_izin, $urut])
                     ->with('message', 'Berhasil Memperbaharui Dokumen Persyaratan.');
        // return redirect()->back()->with('message', 'Berhasil Memperbaharui Dokumen Persyaratan');
    }
    ...
    public function evaluasiUloPostTolak($id, $urut, Request $request)
    {
        ...
        $uloSave->alamat_pelaksanaan_ulo = isset($data['AlamatPelaksanaanUlo']) ? $data['AlamatPelaksanaanUlo'] : '';
        $uloSave->tanggal_evaluasi_pelaksanaan_ulo = isset($data['TanggalEvaluasiPelaksanaanUlo']) ? ($data['TanggalEvaluasiPelaksanaanUlo']) : '';
        $uloSave->no_surat_perintah_tugas = isset($data['NoSuratPerintahTugas']) ? $data['NoSuratPerintahTugas'] : '';
        $uloSave->tanggal_surat_perintah_tugas = isset($data['TanggalSuratPerintahTugas']) ? ($data['TanggalSuratPerintahTugas']) : '';
    ...
    public function evaluasiPenomoranPost($id, $id_kodeakses, Request $request)
    {
        ...
        if ($data['status_sk'] == 0) { //jika ditolak
            ...
            if ($request->hasFile('berkas_tambahan')) {
                // $validatedData = $request->validate([
                //     'berkas_tambahan' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                // ]);
                $file = $request->file('berkas_tambahan');
                // if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                //     $validatedData = $file->validate([
                //         'berkas_tambahan' => [
                //             'required',
                ...
                if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                    ...
                    $file1 = $request->file('berkas_tambahan');
                    ...
                    return redirect()->route('admin.evaluator.evaluasi-penomoran', [$id, $id_kodeakses])->with('message', 'Format File yang diupload tidak sesuai ketentuan.');
                }
    ...
    public function savepencabutanpenomoran(Request $req)
    {
        ...
        // dd($req->all());
        $status_penomoran = 901;
        if ($req->hasFile('berkas_tambahan')) {
            // $validatedData = $req->validate([
            //     'berkas_tambahan' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
            // ]);
            $file = $req->file('berkas_tambahan');
            if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                $validatedData = $req->validate([
                    'berkas_tambahan' => [
                        'required',
            ...
            if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                ...
                $file1 = $req->file('berkas_tambahan');
    ...
    public function savepenetapanulangpenomoran(Request $req)
    {
        ...
        if($req->action=='draft'){
            ...
            // try{
                ...
                if ($req->hasFile('berkas_nib')) {
                    // $validatedData = $req->validate([
                    //     'berkas_nib' => 'required|mimes:pdf|max:5120', // 2048 KB (2 MB) max size
                    // ]);
                    $file = $req->file('berkas_nib');
                    // if (strtolower(($file->getClientOriginalExtension()) === 'pdf')) {
                    //     $validatedData = $file->validate([
                    //         'berkas_nib' => [
                    //             'required',
