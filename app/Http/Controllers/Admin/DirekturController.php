...
class DirekturController extends Controller
{
    ...
    public function index()
    {
        ...
        $ulo = array();
        $ulo = new Ulo();
        $ulo_full = $ulo->view_ulo_aktif($id_departemen_user, 'EMPTY', $id_jabatan);
        // dd($id_departemen_user, $id_jabatan);
        if ($ulo_full != null) { //handle paginate error division by zero
        ...
        $log = DB::table('vw_penomoran_all_aktif as t')->select('t.*')
            ->whereIn('t.status_permohonan', [904, 915])->get();
        // $log = $penomoran;
        ...
        $izinprinsip = IzinAktif::whereIn('status_checklist', [804, 8041, 704, 99903]);
        ...
        $izintelsus = IzinAktif::where('status_checklist', '=', 805);
    ...
    public function penetapanIP($id_izin, Request $request)
    {
        ...
        $izinprinsip = IzinAktif::select('*')->where('id_izin', '=', $id_izin)->first();
    ...
    public function penetapanIPPost($id, Request $request)
    {
        ...
        $izin = IzinAktif::select('*')->where('id_izin', '=', $id)
    ...
    public function penetapanUloPost($id, $urut, Request $request)
    {
        ...
        // // dd($ulo);
        $emailSent = false;
        // dd($penanggungjawab, $email_jenis, $ulo, $departemen, $catatan_hasil_evaluasi, $nama2, $nibs, $koreksi_all, $attachfile, '', $attachfile2, $attachfile3);
        if (!$emailSent) {
            $email->kirim_email($penanggungjawab, $email_jenis, $ulo, $departemen, $catatan_hasil_evaluasi, $nama2, $nibs, $koreksi_all, $attachfile, '', $attachfile2, $attachfile3);
            $emailSent = true;
        }
        // $email->kirim_email($penanggungjawab, $email_jenis, $ulo, $departemen, $catatan_hasil_evaluasi, $nama2, $nibs, $koreksi_all, $attachfile, '', $attachfile2, $attachfile3);
    ...
    public function penetapanPenomoranPost($id, $id_kodeakses, Request $request)
    {
        ...
        if (
            ...
            // dd($penomoran);
            if(isset($penomoran['id_mst_kode_akses'])){
                $penomoran_alokasi =
                ...
                // dd($penomoran['id_izin']);
                
                $penomoran_alokasi =
                    DB::table('tb_trx_kode_akses_alokasi_bloknomor')->select('*')->where('id_izin', '=', $penomoran['id_izin'])->update([
                        'status' => 'Aktif', 'nomor_penetapan' => $no_izin, 'tanggal_penetapan' => date('Y-m-d H:i:s'),
                        'nib' => $nib, 'nama_pelakuusaha' => $nibs['nama_perseroan']
                    ]);
                    $concatenated_izin .= $no_izin . ', ';
        ...
        if ($penomoran['jenis_permohonan'] == 'Pengembalian Penomoran') {
            ...
            $link = 'https://e-telekomunikasi.komdigi.go.id/survei/detail/' . $penomoran['id_izin'];
            $kirim_email = $email->kirim_email(
                $penanggungjawab,
                $email_jenis,
                $penomoran,
    ...
    public function putFileSKPenetapan($id_izin)
    {
        ...
        if (isset($komitmenSK->sk_no)) {
            if ($id_master_izin == 1) {
                ...
                // $latest_nokomitmen = $nokomitmen->izinkomitmenno;
                $pdf = PDF::loadView('layouts.backend.sk.cetak-penetapan', ['ulo' => $ulo, 'latest_nokomitmen' => $komitmenSK->sk_no, 'idizin' => $idizin, 'date_reformat' => $date_reformat, 'nama_perseroan' => $nama_perseroan, 'data' => $data, 'cities' => $cities, 'map_izin_perubahan' => $map_izin_perubahan, 'mst_kode_izin' => $mst_kode_izin]);
            }

            $path = $komitmenSK->path_sk_ulo;
            // DB::table('tb_trx_komitmen_sk')->update(
            //     ['updated_by' => Session::get('id_user'), 'updated_at' => date('Y-m-d H:i:s'), 'is_active' => 1, 'jenis_sk' => 'Penetapan']
    ...
    public function putFileSKIzinPenyelenggaraan($id_izin)
    {
        ...
        $izin = IzinAktif::from('vw_list_izin as u')
            ->select(
                'ip.no_izin_prinsip',
                'ip.tgl_izin_prinsip_init',
                'ip.no_izin_prinsip_ext',
